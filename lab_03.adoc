...
. In the *Boot Dashboard*, right click on `fortune-teller-config-server` and choose *(Re)start*:
+
...
. As before, in the *Boot Dashboard*, right click on `fortune-teller-eureka` and choose *(Re)start*.
...
. As before, in the *Boot Dashboard*, right click on `fortune-teller-fortune-service` and choose *(Re)start*.
...
. In the `fortune-teller-ui` module, create the package `io.spring.cloud.samples.fortuneteller.ui.services.fortunes`.
...
. As before, in the *Boot Dashboard*, right click on `fortune-teller-ui` and choose *(Re)start*.
...
. As before, in the *Boot Dashboard*, right click on `fortune-teller-hystrix-dashboard` and choose *(Re)start*.
